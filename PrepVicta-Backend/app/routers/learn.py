...
from app.services.learn_service import get_chapters, get_user_sections, search_topics, get_topic_by_chapter_section
...
async def chapters(
    ...
    try:
        pool = await get_pool()
        return await get_chapters(subject, pool, user_id=user_id)
...
async def sections(
    ...
    try:
        pool = await get_pool()
        return await get_user_sections(chapter, subject, pool, user_id)
    except Exception as exc:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(exc)) from exc
...
async def topic(
    ...
    try:
        result = await get_topic_by_chapter_section(chapter, section, subject)
        if not result:
